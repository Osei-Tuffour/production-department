...
// Ensure old records have groupSection and noWorkDay fields and add unique IDs
productionRecords = productionRecords.map((rec, index) => ({
    id: rec.id || Date.now() + index + Math.random().toString(36),
...
// Save to localStorage immediately if we added IDs
if (!productionRecords[0]?.id) {
    saveToLocalStorage();
}
    ...
    tbody.innerHTML = productionRecords.map(record => {
        ...
        return `
            <tr class="${rowClass}">
                ...
                <td>
                    <button class="btn-update" onclick="openEditModal('${record.id}')" style="padding: 5px 10px; font-size: 0.9em; margin-right: 5px;">Update</button>
                    <button class="btn-danger" onclick="deleteRecord('${record.id}')" style="padding: 5px 10px; font-size: 0.9em;">Delete</button>
                </td>
            </tr>
        `;
    }).join('');
...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    const originalRecord = productionRecords[recordIndex];

    // Check if new date+group already exists in a DIFFERENT record
    if (newDate !== originalRecord.date || newGroup !== originalRecord.groupSection) {
        const exists = productionRecords.some(record => 
            record.id !== recordId && record.date === newDate && record.groupSection === newGroup
        );
        if (exists) {
...
window.addEventListener('load', function() {
    ...
    // Generate weekly report if on weekly tab
    if (savedTab === 'weekly') {
        generateWeeklyReport();
    }
});
...
window.toggleNoWorkDay = toggleNoWorkDay;
